...
```{R, echo=TRUE, eval=TRUE}
## replace [path]C:/Users/rba/Desktop] with your own project directory path
## on windows machines, replace [\] with [/]
path <- 'C:/Users/Paul/Desktop/MSIB32500-master/lectures'
setwd(path)
...
# Run  gene anotation package (lincra)
*** 
...
1. Try to filter the DEGs with a more stringent cutoff: fdr < 0.01, fold change (fc) >= 1.5 or <= -1.5. How many DEGs are left after filtering?
2. Generate a new gene expression heatmap using significant DEGs from step 1 after applying the new filters. 
3. (bonus point!) For the heatmap from step 2, instead of using the default blue-red color theme, try to change it to green-red instead and re-generate the heatmap.
